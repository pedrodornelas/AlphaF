...
def BEP_asymptotic(L: int, N: int, params: list, p: float, gammaBar: np.array) -> list:
    if N != 2:
        exit()
    ...
    gamma_sum2 = gamma(1/2 + (Ui/2) * L)
    gamma_sum3 = gamma(Ui * L)
    gamma_sum4 = gamma(1 + ((Ui/2) * L))
    psiU = (gamma_sum1*gamma_sum2) / (gamma_sum3*gamma_sum4)

    BEP = (preH * psiU * phiU * (Xi**(Ui*L))) / Bc

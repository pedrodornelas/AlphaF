...
if __name__ == '__main__':
    ...
    preH = 1/2 * (z_PE[0]*z_PE[1])**L / (alpha[0]*alpha[1]*gamma_coef1*gamma_coef2)**L
    ...
    for n in range(N):
        ...
        Psi_Ui = gamma(1-Epsilon3[0]+Epsilon3[1]*L*Ui)/(gamma(1-Epsilon2[0]+Epsilon2[1]*L*Ui)*gamma(1-Epsilon1[0]+Epsilon1[1]*L*Ui))
        Phi_Ui = product(b, Ui, j) * gamma(1-Ai1[0]+Ai1[1]*Ui) * gamma(1-Ai2[0]+Ai2[1]*Ui) * gamma(Ui) / (gamma(Bi1[0]-Bi1[1]*Ui)*gamma(Bi2[0]-Bi2[1]*Ui))
        OP_asymptotic[n] = preH * Psi_Ui * (Phi_Ui**L) * ((Xi[0]*(gamma_th**(1/2)))**(Ui*L)) / (Beta**L)

        end_it = time()
        # print("OP[{:>3}] = {:.8e} (L = {:d} => {:.3f} s)".format(n, OP[n], L, end_it - start_it))
        # print("OP[{:>3}] = {:.8e} (L = {:d} => {:.3f} s)".format(n, OP_asymptotic[n], L, end_it - start_it))

...
def PEP_asymptotic(user: int, L: int, params: list[float], theta: float, gammaBar: np.array) -> list[float]:    
    ...
    if mu < ((z**2) / alpha):
        ...
        frac2 = (gamma(L+1)*gamma((1 + alpha*mu*real_user)/2)) / (gamma(real_user)*gamma(L-real_user+1))
        Psi = mu/(ms-1)
        ...
        frac1 = 1 / (2*real_user*(pi**(1/2)))
        frac2 = (gamma(L+1)*gamma((1 + (z**2)*real_user)/2)) / (gamma(real_user)*gamma(L-real_user+1))
        Psi = mu/(ms-1)

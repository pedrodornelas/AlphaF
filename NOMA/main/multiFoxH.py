...
def parseArgsToMultiH(params: list[float], idx: int, omega: np.array, points: int) -> np.array:
    ...
    # c -> top left
    # d -> bot left
    a = [[(1-ms, 1), ((z**2)/alpha + 1,1)]] + [[(1-ms,1), (1,1), ((z**2)/alpha + 1,1)]]*(idx-1)
    b = [[(mu, 1), ((z**2)/alpha, 1)]] + [[(mu, 1), ((z**2)/alpha, 1), (0, 1)]]*(idx-1)
    c = [tuple([1]+[alpha/2]*(idx)), tuple([1/2]+[alpha/2]*(idx))]
    d = [tuple([0]+[alpha/2]*(idx))]
...
def firstUserMultiH(params: list[float], k: int, omega: np.array, points: int) -> np.array:
    omega = (np.array(omega))
    ...
    # c -> top left
    # d -> bot left
    # a = [(1-ms, 1), ((z**2)/alpha + 1,1)] + [[(1,1)] + Ain + Bin]*(L-1)
    a = [[(1-ms, 1), ((z**2)/alpha + 1,1)]] + [[(1-ms,1), (1,1), ((z**2)/alpha + 1,1)]]*k
    b = [[(mu, 1), ((z**2)/alpha, 1)]] + [[(mu, 1), ((z**2)/alpha, 1), (0, 1)]]*k
    c = [tuple([1]+[alpha/2]*(k+1)), tuple([1/2]+[alpha/2]*(k+1))]
    d = [tuple([0]+[alpha/2]*(k+1))]
    
    # pre allocation
    H = np.zeros(points)

    for i in range(points):
        aux = np.array([omega[i]]*(k+1))
        # print(aux)

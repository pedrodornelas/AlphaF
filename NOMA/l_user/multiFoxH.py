...
def parseArgsToMultiH(params: list[float], l: int, k: int, Xi: np.array, points: int) -> np.array:
    Xi = (np.array(Xi))
    # print('------------ XI -------------')
    # print(Xi)
    ...
    # parameters for FoxH
    mn = [(0, 2)] + [(2, 1)] + [(2, 2)]*(l+k-1)
    pq = [(2, 1)] + [(2, 2)] + [(3, 3)]*(l+k-1)
    # a -> top right
    # b -> bot right
    # c -> top left
    # d -> bot left
    a = [[(1-ms, 1), ((z**2)/alpha + 1,1)]] + [[(1-ms,1), (1,1), ((z**2)/alpha + 1,1)]]*(l+k-1)
    b = [[(mu, 1), ((z**2)/alpha, 1)]] + [[(mu, 1), ((z**2)/alpha, 1), (0, 1)]]*(l+k-1)
    c = [tuple([1]+[alpha/2]*(l+k)), tuple([1/2]+[alpha/2]*(l+k))]
    d = [tuple([0]+[alpha/2]*(l+k))]
